...
int main(int argc, char *argv[])
{
	...
	int iterationtime = ((int) (intervalcount / 301)) +1;
		...
		if(forkedid == 0){
			...
			for(j = 0 ; j < iterationtime; j++){
				...
				if(j < iterationtime -1 ){
					...
					int k;
					for(k = 0 ; k < 301; k++){
						histdata.histogram[k]= histogram[301*j+k].count;
					}
					histdata.start_val = histogram[301*j].start_val;
				}else if(j == iterationtime -1){
					//find modular of 301
					int mod = intervalcount % 301;
					int k;
					if(mod == 0){
...
//------------------------------------------------------
		...
		for(l = 0 ; l < iterationtime; l++){
			printf("iteration time for parent is %d\n",iterationtime-l);
				...
				if(l < iterationtime -1 ){
					...
					int t;
					for(t = 0 ; t < 301; t++){
						histdata2.histogram[t]= histogram[301*l+t].count;
					}
					histdata2.start_val = histogram[301*l].start_val;
				...
				if(l < iterationtime -1){
					int r;
					for(r = 0 ; r < 301; r++){
						printf("before send to client (%d-%d]: %d\n",histogram[l*301+r].start_val,histogram[l*301 +r].end_val,histdata2.histogram[r]);
					}
