...
int main(int argc, char *argv[])
{
   ...
   /* char* fileArr[noOfFile];
    ...
    for(l = 0 ; l < iterationtime; l++){
            ...
            //print histdata
            /*
            printf("start value of iteration: %d is %d\n",j,histdatath.start_val);
            if(l < iterationtime -1){
                int r;
                for(r = 0 ; r < 501; r++){
                    printf("before send to client (%d-%d]: %d\n",histogram[l*501+r].start_val,histogram[l*501 +r].end_val,histdatath.histogram[r]);
                }
                ...
                int r;
                for(r = 0 ; r < mod; r++){
                    printf("in  final histogram before send to client histogram last iteration: :%d\n",histdatath.histogram[r]);
                }
                printf("last value of iteration count is:%d\n",histdatath.histogram[mod]);
            }
